...
Invoke the Skill tool with skill="incremental-implementation" and skill="test-driven-development" (exact names).
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix → invoke the investigate skill (gstack)
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → stop and get explicit user sign-off before continuing
...
If any step fails, invoke the investigate skill (gstack).
